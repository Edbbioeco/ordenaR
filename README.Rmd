...
Now, let's to analyse our data to a direct gradient. We informe our gradient column (`gradient` argument), a column named by `gradient_1`, and species abundance data (`species` argument), a column set ranging from 4th to 8th column in our data frame.
...
Now, we build a barplot to a direct gradient, similar to `ordenaR::order_species()` function.
...
We also can build those plots, but using circles instead bars. Instead bar hight, use use circle diameter to understando species abundance values. Similar to `ordenaR::order_bar()` function, we use `ordenaR::order_circle()` function.
